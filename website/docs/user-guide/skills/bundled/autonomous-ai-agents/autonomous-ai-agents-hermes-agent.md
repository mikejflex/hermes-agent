...
- **Container status confusion**: In Docker or sandboxed environments, `hermes cron status` can be incomplete or misleading across PID namespaces. Corroborate it with the actual gateway container/service state, gateway logs, last cron run timestamps, and persisted health/status files before concluding the scheduler is alive or dead.
